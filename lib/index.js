...
const { toJSON, toXML, createTimeString, wait, versionedApiRequest } = require('./utils')
...
class Modem {
  constructor (options = {}) {
    ...
    this.webUiVersion = this.defaultOptions.webUiVersion ? this.defaultOptions.webUiVersion : undefined;
  ...
  async getToken () {
    ...
    this.setPending(true)
    const response = await got(`http://${this.modemIp}/html/home.htm`)

    const dom = new JSDOM(response.body)
    const session = response.headers['set-cookie'][0].match(/SessionID=(.*);/)[1]
  ...
  async getVersion () {
      if (!this.webUiVersion) {
          await this.waitForPending()
    
          const response = await this.apiRequest('/device/information', {
            method: 'get',
          });
    
          this.webUiVersion = response.WebUIVersion;
    }
  ...
  async getMessages (options = {}) {
    ...
    const response = await this.apiRequest(...versionedApiRequest(this.webUiVersion, '/api/sms/sms-list', {
      method: 'post',
      body: {
        Ascending: ascending,
        BoxType: type,
  ...
  async sendMessage (options = {}) {
    ...
    const response = await this.apiRequest(...versionedApiRequest(this.webUiVersion, '/sms/send-sms', {
      token,
      method: 'post',
      body: {
        Index: '-1',
  ...
  async onMessage (callback) {
    const initialResponse = await this.getMessages({ count: 20, internalFormat: true })
  ...
  async getLastMessage (callback) {
    const [inbox] = await Promise.all([
      this.getMessages({ count: 20, internalFormat: true }),
  ...
  async connectDial () {
    ...
    const response = await this.apiRequest(...versionedApiRequest(this.webUiVersion, '/dialup/dial', {
        method: "POST",
        body: {
            Action: 1,
        }
  ...
  async connect () {
    ...
    const response = await this.apiRequest(...versionedApiRequest(this.webUiVersion, '/dialup/mobile-dataswitch', {
        method: "POST",
        body: {
            dataswitch: 1,
        }
  ...
  async trafficStatistics () {
    ...
    const response = await this.apiRequest(...versionedApiRequest(this.webUiVersion, '/monitoring/traffic-statistics', {
        method: "GET",
    }));
  ...
  async status () {
    ...
    const response = await this.apiRequest(...versionedApiRequest(this.webUiVersion, '/monitoring/status', {
        method: "GET",
    }));
